...
export default function AllTasks({ allTasks,teams }) {
  ...
  const myTasks = allTasks.filter(
    (task) => task.team_owner_email === user?.email
  );
 ...
 const  handleEditModal = (task) => {
  ...
  return (
              ...
              {myTasks.length > 0 ? (
                myTasks.map((task, index) => (
                  ...
                  >
                    ...
                    <td className="p-4 border-b border-gray-700/50">
                      {task.assigned_member.member_name ? task.assigned_member.member_name : task.assigned_member}
                    </td>
    ...
    font-medium px-3 py-2 rounded-lg flex items-center gap-2 transition-colors cursor-pointer
                      "
                      >
                        <MdDelete className="text-lg text-white font-bold" />
                        Delete
                      </button>
                    </td>
                  </tr>
